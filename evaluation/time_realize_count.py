...
def get_transition_times(agent_p, total_target, file_num):
    ...
    first_reach = [False for _ in range(NUM_AGENTS)]
    ...
    for gen in range(file_num):
        ...
        for t in range(MAX_TIME):
            for i in range(NUM_AGENTS):
                ...
                if agent.inspire == heatmap[int(agent.coord.x)][int(agent.coord.y)] and not first_reach[i]:
                    first_reach[i] = True
                if not first_reach[i]:
                    time_count[i] += 1  # Calculate the timeStep for each agent only if it hasn't reached MEDIUM yet
...
if __name__ == "__main__":
    ...
    save_times_to_csv(time_counts, "F://self//evaluation//csv_files//realize_result//TAMS_agent_time_counts.csv")
