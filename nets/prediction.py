...
class Prediction:
    def __init__(self, input, hidden, output):
        ...
        self.weight_predictionNet_layer0 = np.zeros((REPETITION, POP_SIZE, PRE_CONNECTIONS), dtype=float)
        self.weight_predictionNet_layer1 = np.zeros((REPETITION, POP_SIZE, (self.input + 1) * self.hidden), dtype=float)
        self.weight_predictionNet_layer2 = np.zeros((REPETITION, POP_SIZE, self.output * self.hidden), dtype=float)

        # hidden states prediction network
        self.hiddenBefore_predictionNet = np.zeros((REPETITION, NUM_AGENTS, hidden), dtype=float)

        # predictions of agents FOR Proximity sensors
        self.predictions = np.zeros((REPETITION, NUM_AGENTS, SENSORS - 1), dtype=float)
    ...
    def propagate_prediction_network(self, layer0, layer1, layer2, agent, input, zone, rep):
        """
        ...
        hidden_net += self.hiddenBefore_predictionNet[rep][agent] * input_2_hidden_2[self.input,
                                                                    :]  # Recurrent connections

        # Update hiddenBefore_predictionNet with activated values
        self.hiddenBefore_predictionNet[rep][agent] = self.sigmoid(hidden_net)  # FIX: added [rep]
        ...
        for j in range(SENSORS - 1):
            ...
            if zone == MEDIUM:
                ...
                self.predictions[rep][agent][j] = net[j] if -0.03 <= float(result) - net[j] <= 0.03 else float(result)
            elif zone == LOW:
